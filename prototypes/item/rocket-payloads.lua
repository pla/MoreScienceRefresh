...
local itemInCrateName = settings.startup["MS-rocket-launching-extended"].value
    and string.format(scienceNames.mixing, "pack")
  or "empty-bottle"
...
local rocketPayloadIcons =
  LSlib.item.getIcons("item", string.format(scienceNames.white, "pack"), 0.48 * 2, { 0, 16 }, nil)
for _, iconLayer in pairs(rocketPayloadIcons or {}) do
  table.insert(rocketPayload.icons, iconLayer)
end
